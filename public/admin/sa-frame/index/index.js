...
var homeTab = {
	id: '1-1',	// 唯一标识 
	name: '概要',
	url: 'sa-view/console/overview.html',	// 页面地址 
	isNeedLoad: false,		// 标注：是否需要此刻加载
	hideClose: true	// 隐藏关闭键 
}

const CACHE_MENUS = "CACHE_MENUS"

function getCache() {
	try {
		return JSON.parse(localStorage.getItem(CACHE_MENUS)) || [homeTab]
	} catch (error) {
		return [homeTab]
	}
}

function saveCache(menus) {
	localStorage.setItem(CACHE_MENUS, JSON.stringify(menus.map(item => {
...
var sa_admin = new Vue({
	...
	data: {
		// ------------------------------- 配置 -------------------------------
		title: sa.app_name,		// 页面标题  -- Sa-Admin
		logo: sa.app_logo,		// logo地址  -- sa-frame/admin-logo.png
		...
		activeMenuId: '1-1',		// 正在高亮的菜单id 
		isDrag: false,			// 当前是否正在拖拽 tab 
		dragTab: null,			// 当前正在拖拽的 tab 
		tabList: getCache(),		// 当前 Tab 集合 
		viewList: getCache(),		// 当前 View 集合 
	...
	methods: {
		...
		async getDraftContent() {
			const res = await sa.http({
				url: '/contents/latestDraft'
			})
			if (res && res.data && res.data.cid) {
				const title = res.data.title || ''
				sa_admin.updateMenuById('2#2-1-0', {
					isShow: true,
					// 菜单文案：继续编辑xxx文章
					name: title ? `继续编辑${title}文章` : '继续编辑文章',
					params: new URLSearchParams(res.data).toString()
				})
			} else {
				sa_admin.updateMenuById('2#2-1-0', {
					isShow: false,
				...
				this.viewList = this.viewList.filter(item => item.id !== '2-1-0')
				this.tabList = this.tabList.filter(item => item.id !== '2-1-0')
			}
		},

		updateMenuById(id_paths, data) {
			const ids = id_paths.split('#')
			let menu = this.menuList
			let res
			for (let i = 0; i < ids.length; i++) {
				res = this.findTargetMenu(ids[i], menu);
				if (res.childList) {
					menu = res.childList
				}
			}
			...
			Object.keys(data).forEach(key => {
				Vue.set(res, key, data[key])
			})

			console.log(this.menuList)
		},

		findTargetMenu(id, menus) {
		...
		// 显示某个菜单，根据id 
		showMenuById: function (id, params) {
			var menu = this.getMenuById(id);
			if (menu) {
				this.showTab(menu, params || menu.params);
			}
		},
		// 显示homeTab
		showHome: function () {
		...
		updateTabByUser(id, params) {
			var tab = this.getMenuById(id);
			if (tab) {
				// 标注：需要此刻加载 
				// tab.isNeedLoad = false;	
				Vue.set(tab, 'isNeedLoad', true);

				Vue.set(tab, 'params', params)

				this.tabList = this.tabList.map(item => {
					return item.id === tab.id ? tab : item
				...
				const targetTab = this.getTabById(tab.id)
				if (targetTab == null) {
					this.addTab(tab);
				}
				// 然后开始显示这个 tab 
		...
		showTab: function (tab) {
			...
			if (!tab.timestamp) {
				Vue.set(tab, 'timestamp', Date.now())
			}
		...
		closeTab: function (tab, callFn) {
			...
			setTimeout(function () {
				...
				// 从 tabList 中移除这个 tab 
				this.tabList = this.tabList.filter(_item => {
					return _item.id !== tab.id
				})

				this.viewList = this.viewList.filter(_item => {
					return _item.id !== tab.id
				})
		...
		// 根据当前tab刷新一下锚链接 
		f5HashByNativeTab: function (params) {
			// 如果非记住模式
			if (this.isRemeOpen == false) {
				return;
			}
		...
		initUserInfo() {
			try {
				return JSON.parse(localStorage.getItem('_user_info_')) || ''
			} catch (error) {
				return ''
			}
		}
