<template>
	<div class="view-vessel">
		<div class="a-view" v-for="tab in $root.viewList" :key="tab.id"
			:class="tab.id == $root.nativeTab.id ? 'a-view-native' : null">
			<!-- vue视图 -->
			...
			<!-- iframe视图 -->
			<template v-else>
				<iframe :src="tab.url + '?v=' + tab.timestamp + '&' + tab.params" :id="'iframe-' + tab.id" v-if="tab.isNeedLoad"
					:key="tab.params ? tab.id + tab.params : tab.id" @load="onloadIframe(tab.id)"></iframe>
			</template>
		</div>
		<!-- tab被拖拽时的遮罩（下托拽：悬浮打开） -->
...
.a-view {
	...
	background: url(../index/admin-loading.gif) no-repeat center 50%;
	position: absolute;
}
