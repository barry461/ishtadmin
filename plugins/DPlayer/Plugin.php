...
class Plugin extends PluginInterface
{
    ...
    {
        Helper::addRoute('danmaku', '/danmaku/[id].json', 'DPlayer_Action', 'comments');
    ...
    public static function playerHeader()
    {
        ...
        echo <<<EOF
<link rel="stylesheet" type="text/css" href="$url/assets/DPlayer.min.css?v=1" />
EOF;
    }
    ...
    public static function playerFooter()
    {
        ...
        $url = '/usr/plugins/DPlayer';
        $url = parse_url($url , PHP_URL_PATH);
        if ($options->hls) {
            echo "<script type=\"text/javascript\" src=\"$url/plugin/hls.min.js?v=3\"></script>\n";
        }
        if ($options->flv) {
            echo "<script type=\"text/javascript\" src=\"$url/plugin/flv.min.js?v=3\"></script>\n";
        }
        echo <<<EOF
<script type="text/javascript" src="$url/assets/DPlayer.min.js?v=3"></script>
<script type="text/javascript" src="$url/assets/player.js?v=3"></script>
EOF;
    }
...
//        debug_print_backtrace(DEBUG_BACKTRACE_IGNORE_ARGS);
    ...
    public static function parsePlayer($attrs)
    {
        ...
        $open_danmaku = Typecho_Widget::widget('Widget_Options')->plugin('DPlayer')->danmaku;
        if (false !== strpos($attrs['url'] , '.m3u8')){
            $attrs['type'] = 'hls';
        }
        $video_ads_url = Typecho_Widget::widget('Widget_Options')->plugin('DPlayer')->video_ads_url;
        $ads_jump_url = Typecho_Widget::widget('Widget_Options')->plugin('DPlayer')->ads_jump_url;
        $ads_jump_time = Typecho_Widget::widget('Widget_Options')->plugin('DPlayer')->ads_jump_time;
        ...
        $config = [
            ...
            'mutex' => true,
            'video_ads_url' => empty($video_ads_url) ? '' : trim($video_ads_url),
            'ads_jump_url' => empty($ads_jump_url) ? '' : replace_share($ads_jump_url),
            'ads_jump_time' => (int)(empty($ads_jump_time) ? -1 : $ads_jump_time),
            'video' => [
    ...
    public static function config($form)
    {
        ...
        $hls = new Typecho_Widget_Helper_Form_Element_Radio('hls', array('0' => _t('不开启HLS支持'), '1' => _t('开启HLS支持')), '1', _t('HLS支持'), _t("开启后可解析 m3u8 格式视频"));
        $flv = new Typecho_Widget_Helper_Form_Element_Radio('flv', array('0' => _t('不开启FLV支持'), '1' => _t('开启FLV支持')), '0', _t('FLV支持'), _t("开启后可解析 flv 格式视频"));
        $danmaku = new Typecho_Widget_Helper_Form_Element_Radio('danmaku', array('0' => _t('不开启评论弹幕'), '1' => _t('开启评论弹幕')), '0', _t('是否开启评论弹幕'), _t("开启后可显示评论弹幕"));
        $video_ads_url = new Typecho_Widget_Helper_Form_Element_Textarea(
            'video_ads_url', null, '',
            _t('视频广告地址'));
        $ads_jump_url = new Typecho_Widget_Helper_Form_Element_Text(
            'ads_jump_url', null, '',
            _t('广告跳转地址'), _t('支持 https://{share.xxx}'));
        $ads_jump_time = new Typecho_Widget_Helper_Form_Element_Text(
            'ads_jump_time', null, '',
            _t('广告跳过时长'), _t('默认不可跳过'));
        $form->addInput($theme);
        $form->addInput($api);
        $form->addInput($video_ads_url);
        $form->addInput($ads_jump_url);
        $form->addInput($ads_jump_time);
        $form->addInput($hls);
        $form->addInput($flv);
