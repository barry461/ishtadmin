    ...
    ,'<div class="layim-chat-footer">'
      ...
      ,'<div class="layim-chat-bottom">'
        ,'<div class="layim-chat-send">'
          ,'<span class="layim-send-close" style="background-color: #FFB800;" data-method="notice" id="order_message" onclick="dd({{d.data.id}},'+"'{{d.data.name}}'"+');">查看订单</span>'
          ,'{{# if(!d.base.brief){ }}'
          ,'<span class="layim-send-close" layim-event="closeThisChat">关闭窗口</span>'
          ,'{{# } }}'
  ...
  var anim = 'layui-anim-up', events = {
    ...
    ,chatLog: function(othis){
      ...
      return events.chatLog.index = layer.open({
        ...
        ,content: cache.base.chatLog + '?id=' + thatChat.data.id + '&type=' + thatChat.data.type+'&sid='+cache.mine.id
      });
    }
