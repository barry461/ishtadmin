(async function (doc, win, layer, sbar) {
    ...
    const init = () => {
            ...
            return async (i = -1) => {
                try {
                    ...
                    if (data === null) {
                        ...
                        if (!response.ok) {
                            throw new Error(`评论数据获取失败, 状态码: ${response.status}`);
                        }
                        data = await response.json(); // 存入缓存
                    }
        
                    // 2. 处理数据
                    if (i > -1) {
                        if (!data[i]) throw new Error(`父评论索引 ${i} 不存在`);
                        if (!data[i]?.children) throw new Error(`子评论索引 ${i} 不存在`);
        ...
        const template = (c) => {
            return `
                <div class="comment-author" itemprop="creator" itemscope="" itemtype="http://schema.org/Person">
                    <span itemprop="image">
                        <img class="avatar" src="../../usr/themes/Mirages/images/911cg.png?s=100&amp;r=G&amp;d=" data-src="" alt="${c.author}" width="100" height="100">
                    </span>
                    <cite class="fn color-main" itemprop="name">${c.author}</cite>
                </div>
                <div class="comment-reply">
                    <a href="javascript:;" rel="nofollow" onclick="return UserComment.reply(event, '${c.coid}', ${c.parent});">回复</a>
                </div>
                <div class="comment-meta">
                    <a href="javascript:;">
                        <time itemprop="commentTime" datetime="${c.date}">${c.date}</time>
                    </a>
                </div>
        ...
        new PagetionList({
            ...
            itemRenderer: (c, index) => {
                return `
                    ...
                    <div class="comment-reply">
                        <a href="javascript:void(0);" rel="nofollow" onclick="return UserComment.reply(event, '${c.coid}', ${c.parent});">回复</a>
                    </div>
                    
                    <div class="comment-meta">
                        <a href="javascript:;" class="no-animation">
                        <time itemprop="commentTime" datetime="${c?.date}">${c?.date}</time>
                        </a>
                    </div>
                    ...
                    <div class="comment-children" itemprop="discusses">
                        <ol class="comment-list comment-children-ol">
                            ${(c?.children.slice(0, 3)).map(c => `
                                <li itemscope="" itemtype="http://schema.org/UserComments" id="comment-${c.coid}" class="comment-body comment-child comment-level-odd comment-odd">
                                    <div class="comment-author" itemprop="creator" itemscope="" itemtype="http://schema.org/Person">
                                        <span itemprop="image">
                                            <img class="avatar" src="../../usr/themes/Mirages/images/911cg.png?s=100&amp;r=G&amp;d=" data-src="" alt="${c.author}" width="100" height="100">
                                        </span>
                                    ...
                                    <div class="comment-reply">
                                        <a href="javascript:;" rel="nofollow" onclick="UserComment.reply(event, '${c.coid}', ${c.parent});">回复</a>
                                    </div>
                                    <div class="comment-meta">
                                        <a href="javascript:;">
                                            <time itemprop="commentTime" datetime="${c.date}">${c.date}</time>
                        ...
                        ${(c?.children?.length <= 3) ? `` : `
                            <div class="more-child-comment-btns">
                                ...
                                <div class="flex show-more-child-comment">
                                    <a href="javascript:;" class="content" onclick="return onTypechoOpenChildComment(event, ${index}, this);">展开${c?.children?.length || 0}条回复</a>
                                    <div class="xqbj-icon-arrow"></div>
                                </div>
                                <div class='flex hide-more-child-comment' onclick="return onTypechoCloseChildComment(${index}, this);">
            ...
            dataFetcher: async () => getDate()
        });
        
        // 关闭弹窗
        window.onTypechoCloseChildComment = async (index = -1, _this = null) => {
            ...
            if(index > -1 && _this) {
                const childalldata = await getDate(index);
                ...
                // 更新提示
                commentChildrenText.textContent = `展开${childalldata?.length}条回复`;
        ...
        // 打开弹窗
        window.onTypechoOpenChildComment = async (e, index, _this) => {
            e.preventDefault();
            ...
            if(document?.documentElement?.clientWidth > 750) {
                ...
                for (let i = 0; i < childrendata.length; i++) {
                    ...
                    li.id = `comment-${childrendata[i].id}`;
            ...
            // 弹窗处理
                ...
                win['virtualList'] = new VirtualList({
                    ...
                    estimatedItemHeight: 93.19, // 更接近实际高度的估计值
                    bufferItems: 50, // 适当增加缓冲数量
                    itemsPerGroup: 4, // 每组4个项目
                    bufferGroups: 2, // 预加载前后2组
    ...
    class UserComment {
        ...
        submitReply(e) {
            ...
            // 表单数据
            const data = { text, author, ...(cid && cid > 0 ? { parent: cid } : {}) };
    ...
    class ScrollLock {
    	...
    	unlock() {
    		...
    		const scrollY = Math.abs(parseInt(
    			document.body.style.getPropertyValue('--scroll-lock-position') || '0'
    		));
    ...
    class PagetionList {
        constructor(options) {
            ...
            this.commentlist = [];
            this.currentPage = 1;
            this.itemsPerPage = this.config.itemsPerPage; // Use the configured value
            this.totalPages = 0;
        ...
        // 构建列表
        async build(start = 0, end = this.itemsPerPage - 1) {
            this.currentPage = Math.floor(start / this.itemsPerPage) + 1;
            this.totalPages = Math.ceil(this.commentlist.length / this.itemsPerPage);
            this.list.innerHTML = "";
            
            const fragment = document.createDocumentFragment();
            for (let i = start; i <= end && i < this.commentlist.length; i++) {
                const li = document.createElement("li");
                li.id = `comment-${this.commentlist[i].coid}`;
                li.className = "comment-body comment-parent comment-odd";
                li.innerHTML = await this.config.itemRenderer(this.commentlist[i], i);
                fragment.appendChild(li);
            }
            await this.list.appendChild(fragment);
    ...
    class VirtualList {
        constructor(options) {
            ...
            this.groupHeightCache = []; // 每组高度缓存
            this.offsetCache = [0]; // 组偏移量缓存
        ...
        getCurrentGroup() {
            ...
            // 使用二分查找确定当前组
            let low = 0, high = this.totalGroups - 1;
            while (low <= high) {
        ...
        getVisibleRange() {
            ...
            if (currentGroup === this.currentGroup) return null;
        ...
        async renderVisibleGroups() {
            ...
            try {
                ...
                Array.from(this.list.children).forEach(node => {
                    ...
                    if (groupId) existingNodes.set(groupId, node);
                });
        ...
        destroy() {
            ...
            if (this.virtualElement) {
                this.virtualElement.style.height = '0'; // 重置虚拟高度
            }
