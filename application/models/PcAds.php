 ...
 * @property int $oauth_type 广告设备类型 0所有 1macos 2 windows
 ...
 * @mixin \Eloquent
 */
class PcAdsModel extends BaseModel
{
    ...
    const DEVICE_ALL = 0;
    const DEVICE_MACOS = 1;
    const DEVICE_WINDOWS = 2;
    const DEVICE_TYPE = [
        self::DEVICE_ALL     => '所有',
        self::DEVICE_MACOS     => 'macos',
        self::DEVICE_WINDOWS => 'windows',
    ];

    const REDIS_ADS_KEY = 'pc:ads:positions_';
    ...
    public static function listPos($pos, $limit = 15)
    {
        ...
        $key = 'pc:ad:list:' . $pos . '-' . $limit. ':' . $version;;
        return cached($key)
            ->chinese('PC广告列表')
            ->fetchPhp(function () use ($pos, $limit, $version) {
                return self::where('position', $pos)
                    ...
                    ->when(version_compare($version, '2.6.0', '<'), function ($q){
                        $q->whereIn('type', [1, 2]);
                    })
                    ->limit($limit)
                    ->orderByDesc('sort')
