...
class PcadsController extends BackendBaseController
{
    ...
    {
        return function (PcAdsModel $item) {
    ...
    {
        $tips = [];
        foreach (PcAdsModel::POSITION as $k => $v) {
            $tips[$k] = $v . '---' . (PcAdsModel::SIZE_TIPS[$k] ?? '');
        }
    ...
    public function batch_replaceAction(): bool
    {
        try {
            ...
            $record = PcAdsModel::where('url_config', $from)->first();
            test_assert($record, '未找到此原始网址' . $from);

            $isOk = PcAdsModel::where('url_config', $from)->update(['url_config' => $to,]);
            test_assert($isOk, '系统异常');

            CacheKeysModel::where('name', 'Pc广告列表')->chunkById(1000, function ($items) {
