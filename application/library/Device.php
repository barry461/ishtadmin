...
class Device
{
    ...
    public static function isSpider()
    {
        return self::is(array(
            "Spider", "YodaoBot", "Googlebot", "Bingbot", "Slurp", "MSNBot",
            "DuckDuckBot", "YandexBot", "yahoo-blogs", "psbot", "ia_archiver",
        ));
    }
    ...
    {
        //Mac OS X 10_11_4
        if (preg_match('/^.+Mac\s+OS\s+X\s+\d+[^a-zA-Z0-9]+(\d+).*$/i',
            self::getUserAgent(), $matches)) {
    ...
    {
        //Windows NT 6.1
        if (preg_match('/^.*Windows\s+NT\s+([0-9\.]+).*$/i',
            self::getUserAgent(), $matches)) {
    ...
    {
        return self::check(array("Safari", "Version/"), true)
            && !self::check(array("Chrome", "Opera", "OPR", "QQ"), false);
    }
    ...
    public static function is($is, $not = array(), $needAllMatch = false)
    {
        ...
        return self::check($is, $needAllMatch)
            && !self::check($not, $needAllMatch);
    }

    public static function shouldEnableBlurFilter()
    {
        if (self::check(array("Firefox"))
            || (self::is(array("Chrome"), array("Edge")))) {
            return false;
        }
        if (self::isMobile()) {
