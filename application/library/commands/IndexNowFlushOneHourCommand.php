<?php

namespace commands;

class IndexNowFlushOneHourCommand
{
    public $signature = 'indexnow:flush';
    public $description = '一次性推送数据库中一小时前发布的文章URL';

    protected $indexnowHost;
    protected $indexnowKey;
    protected $indexnowKeyLocation;
    protected $maxBatchSize = 10000; // IndexNow API 最大支持10000个URL
    protected $delayBetweenBatches = 1; // 批次间延迟（秒）
    protected $dbChunkSize = 1000; // 数据库分批查询大小

    protected $isPushHome = false;
    protected $isPushKey = "is_push_home";

    protected $deletePostCacheKey = "indexnow:urlset";

    public function handle($argv)
    {
        if (!empty($argv)) {
            $chunkSize = (int)$argv;
            if ($chunkSize > 0) {
                $this->dbChunkSize = $chunkSize;
            }
        }
        if (redis()->get($this->isPushKey) == "1") {
            $this->isPushHome = true;
        }
        $this->initConfig();

        try {
            $totalCount = $this->getTotalPublishedCountByOneHourAgo();
            echo date('Y-m-d H:i:s')." "."数据库中共有 {$totalCount} 篇已发布文章" . PHP_EOL;

            if ($totalCount === 0) {
                echo date('Y-m-d H:i:s')." ".json_encode(['ok' => true, 'message' => '没有已发布的文章'], JSON_UNESCAPED_UNICODE) . PHP_EOL;
            } else {
                $results = $this->processOneHourAgoArticlesInChunks();
                echo date('Y-m-d H:i:s')." ".json_encode($results, JSON_UNESCAPED_UNICODE) . PHP_EOL;
            }


            //推送删除文章
            $deleteResults = $this->processDeletedArticles();
            echo date('Y-m-d H:i:s')." ".json_encode($deleteResults, JSON_UNESCAPED_UNICODE) . PHP_EOL;

            //推送首页（一天只推送一次）
            $homeResults = $this->pushHomePage();
            echo date('Y-m-d H:i:s')." ".json_encode($homeResults, JSON_UNESCAPED_UNICODE) . PHP_EOL;


        } catch (\Throwable $exception) {
            echo date('Y-m-d H:i:s')." ".json_encode([
                    'ok' => false,
                    'error' => $exception->getMessage(),
                    'trace' => $exception->getTraceAsString()
                ], JSON_UNESCAPED_UNICODE) . PHP_EOL;
        }
    }

    protected function initConfig(): void
    {
        $this->indexnowKey = (string)setting('index_now_key');
        $this->indexnowHost = (string)setting('index_now_host');

        if ($this->indexnowHost && $this->indexnowKey) {
            $this->indexnowKeyLocation = 'https://' . $this->indexnowHost . '/' . $this->indexnowKey . '.txt';
        } else {
            $this->indexnowKeyLocation = '';
        }
    }

    /** 从根目录的 key 文件读取 key */
    private function loadKey()
    {
        $path = APP_PATH . 'public/www/' . $this->indexnowKey . '.txt';

        if (!is_file($path)) {
            throw new \RuntimeException("IndexNow key file not found at: {$path}");
        }

        return trim(file_get_contents($path));
    }

    protected function getTotalPublishedCountByOneHourAgo(): int
    {
        return \ContentsModel::where('status', \ContentsModel::STATUS_PUBLISH)
            ->where('type', \ContentsModel::TYPE_POST)
            ->whereBetween("modified", [time() - 3600, time()])
            ->count();
    }


    protected function getPublishedArticlesOneHourAgoChunk(int $offset, int $limit): array
    {
        return \ContentsModel::where('status', \ContentsModel::STATUS_PUBLISH)
            ->where('type', \ContentsModel::TYPE_POST)->with('categories')
            ->whereBetween("modified", [time() - 3600, time()])
            ->select(['cid', 'slug', 'type', 'text'])
            ->offset($offset)
            ->limit($limit)
            ->get()
            ->toArray();
    }

    /** 分批处理所有文章 */
    protected function processOneHourAgoArticlesInChunks(): array
    {
        if (!$this->indexnowKey || !$this->indexnowHost) {
            return ['ok' => false, 'error' => 'missing indexnow host/key'];
        }

        $totalCount = $this->getTotalPublishedCountByOneHourAgo();
        $totalProcessed = 0;
        $totalSuccess = 0;
        $totalFailed = 0;
        $batchResults = [];
        $batchNumber = 1;
        $offset = 0;

        while ($offset < $totalCount) {
            echo "查询第 " . ceil(($offset + 1) / $this->dbChunkSize) . " 批数据库记录..." . PHP_EOL;

            // 从数据库分批查询文章
            $articles = $this->getPublishedArticlesOneHourAgoChunk($offset, $this->dbChunkSize);

            if (empty($articles)) {
                break;
            }

            echo "获取到 " . count($articles) . " 篇文章，开始处理..." . PHP_EOL;

            // 将文章按API限制分批处理
            $articleChunks = array_chunk($articles, $this->maxBatchSize);

            foreach ($articleChunks as $chunk) {
                echo "处理第 {$batchNumber} 批次，共 " . count($chunk) . " 篇文章..." . PHP_EOL;

                $urls = [];
                foreach ($chunk as $article) {
                    if ($this->isArticleContentEmpty($article)) {
                        echo "跳过内容为空的文章 CID {$article['cid']}" . PHP_EOL;
                        continue;
                    }

                    $articleUrl = $this->generateArticleUrl($article);
                    if ($articleUrl) {
                        $urls[] = $articleUrl;
                    }

                    foreach ($article['categories'] as $category) {
                        $categoryUrl = $this->generateCategoryUrl($category);

                        if ($categoryUrl) {
                            $urls[] = $categoryUrl;
                        }
                    }
                }

                $batchResult = $this->pushUrlBatch($urls);

                $totalProcessed += count($urls);

                if ($batchResult['ok']) {
                    $totalSuccess += count($urls);
                    echo "第 {$batchNumber} 批次成功推送 " . count($urls) . " 个URL" . PHP_EOL;
                } else {
                    $totalFailed += count($urls);
                    echo "第 {$batchNumber} 批次推送失败: {$batchResult['error']}" . PHP_EOL;
                }

                $batchResults[] = [
                    'batch' => $batchNumber,
                    'count' => count($urls),
                    'success' => $batchResult['ok'],
                    'http_code' => $batchResult['http'] ?? 0,
                    'error' => $batchResult['error'] ?? null
                ];

                $batchNumber++;

                // 批次间延迟，避免API限制
                if ($this->delayBetweenBatches > 0) {
                    echo "等待 {$this->delayBetweenBatches} 秒后处理下一批次..." . PHP_EOL;
                    sleep($this->delayBetweenBatches);
                }
            }

            $offset += $this->dbChunkSize;

            // 显示总体进度
            $progress = min(100, round(($offset / $totalCount) * 100, 2));
            echo "总体进度: {$progress}% ({$offset}/{$totalCount})" . PHP_EOL;
        }

        return [
            'ok' => $totalFailed === 0,
            'summary' => [
                'total_articles' => $totalCount,
                'total_processed' => $totalProcessed,
                'total_success' => $totalSuccess,
                'total_failed' => $totalFailed,
                'total_batches' => $batchNumber - 1
            ],
            'batch_details' => $batchResults
        ];
    }

    /** 推送URL批次到IndexNow */
    protected function pushUrlBatch(array $urls): array
    {
        if (empty($urls)) {
            return ['ok' => true, 'count' => 0];
        }

        trigger_log(date('Y-m-d H:i:s') . " indexnow batch URLs: " . var_export($urls, true));

        $payload = json_encode([
            'host' => $this->indexnowHost,
            'key' => $this->loadKey(),
            'keyLocation' => $this->indexnowKeyLocation,
            'urlList' => array_values(array_unique($urls)),
        ], JSON_UNESCAPED_SLASHES);

        trigger_log(date('Y-m-d H:i:s') . " indexnow batch payload: " . var_export($payload, true));

        $ch = curl_init('https://api.indexnow.org/indexnow');
        curl_setopt_array($ch, [
            CURLOPT_POST => true,
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_HTTPHEADER => ['Content-Type: application/json'],
            CURLOPT_POSTFIELDS => $payload,
            CURLOPT_TIMEOUT => 30, // 增加超时时间
            CURLOPT_CONNECTTIMEOUT => 10,
        ]);

        $resp = curl_exec($ch);
        $http = curl_getinfo($ch, CURLINFO_HTTP_CODE);
        $err = curl_error($ch);
        curl_close($ch);

        if ($http >= 200 && $http < 300) {
            return [
                'ok' => true,
                'count' => count($urls),
                'http' => $http,
                'resp' => $resp
            ];
        }

        $errorMsg = "IndexNow push failed http={$http} err={$err} resp={$resp}";
        error_log($errorMsg);

        return [
            'ok' => false,
            'count' => count($urls),
            'http' => $http,
            'error' => $err ?: "HTTP {$http}",
            'resp' => $resp
        ];
    }

    /** 推送首页URL（一天只推送一次） */
    protected function pushHomePage(): array
    {
        // 检查今天是否已经推送过首页
        if ($this->isPushHome === true) {
            return [
                'ok' => true,
                'message' => '今天已经推送过首页',
                'type' => 'homepage',
                'already_pushed' => true
            ];
        }

        if (!$this->indexnowKey || !$this->indexnowHost) {
            return ['ok' => false, 'error' => 'missing indexnow host/key'];
        }

        // 生成首页URL
        $siteUrl = rtrim(options('siteUrl'), '/');
        $homeUrl = $siteUrl . '/';

        echo "推送首页URL: {$homeUrl}" . PHP_EOL;

        // 直接复用pushUrlBatch方法
        $result = $this->pushUrlBatch([$homeUrl]);

        if ($result['ok']) {
            // 推送成功后设置Redis标记，TTL为今日剩余时间
            $ttl = strtotime('tomorrow') - time();
            redis()->set($this->isPushKey, 1, $ttl);
            $this->isPushHome = true;

            echo "首页推送成功" . PHP_EOL;

            return [
                'ok' => true,
                'type' => 'homepage',
                'url' => $homeUrl,
                'message' => '首页推送成功'
            ];
        } else {
            echo "首页推送失败: {$result['error']}" . PHP_EOL;

            return [
                'ok' => false,
                'type' => 'homepage',
                'url' => $homeUrl,
                'error' => $result['error'],
                'http_code' => $result['http'] ?? 0
            ];
        }
    }

    /** 生成文章URL */
    protected function generateArticleUrl(array $article): ?string
    {
        $siteUrl = rtrim(options('siteUrl'), '/');

        if ($article['type'] === 'page') {
            $slug = trim((string)($article['slug'] ?? ''));
            if ($slug === '') {
                return null;
            }

            return $siteUrl . '/' . $slug . '/';
        }

        return $siteUrl . '/archives/' . $article['cid'] . '/';
    }

    protected function generateCategoryUrl(array $category): ?string
    {
        $siteUrl = rtrim(options('siteUrl'), '/');
        $slug = trim((string)($category['slug'] ?? ''));
        if ($slug === '') {
            return null;
        }

        return $siteUrl . '/category/' . $slug . '/';
    }

    protected function isArticleContentEmpty(array $article): bool
    {
        if (!array_key_exists('text', $article)) {
            return true;
        }

        $text = $article['text'];

        if (!is_string($text)) {
            return true;
        }

        return trim(strip_tags($text)) === '';
    }

    /** 处理已删除文章的URL推送 */
    protected function processDeletedArticles(): array
    {
        if (!$this->indexnowKey || !$this->indexnowHost) {
            return ['ok' => false, 'error' => 'missing indexnow host/key'];
        }

        // 从Redis Set中获取所有已删除的文章URL
        $deletedUrls = redis()->smembers($this->deletePostCacheKey);

        if (empty($deletedUrls)) {
            return [
                'ok' => true,
                'message' => '没有需要推送的删除文章',
                'count' => 0
            ];
        }

        echo "发现 " . count($deletedUrls) . " 个已删除的文章URL需要推送" . PHP_EOL;

        // 分批处理删除的URL
        $urlChunks = array_chunk($deletedUrls, $this->maxBatchSize);
        $totalSuccess = 0;
        $totalFailed = 0;
        $batchResults = [];
        $batchNumber = 1;

        foreach ($urlChunks as $chunk) {
            echo "推送删除文章第 {$batchNumber} 批次，共 " . count($chunk) . " 个URL..." . PHP_EOL;

            $batchResult = $this->pushUrlBatch($chunk);

            if ($batchResult['ok']) {
                $totalSuccess += count($chunk);
                echo "第 {$batchNumber} 批次成功推送 " . count($chunk) . " 个删除URL" . PHP_EOL;

                // 推送成功后从Redis Set中移除这些URL
                foreach ($chunk as $url) {
                    redis()->srem($this->deletePostCacheKey, $url);
                }
            } else {
                $totalFailed += count($chunk);
                echo "第 {$batchNumber} 批次推送失败: {$batchResult['error']}" . PHP_EOL;
            }

            $batchResults[] = [
                'batch' => $batchNumber,
                'count' => count($chunk),
                'success' => $batchResult['ok'],
                'http_code' => $batchResult['http'] ?? 0,
                'error' => $batchResult['error'] ?? null
            ];

            $batchNumber++;

            // 批次间延迟
            if ($this->delayBetweenBatches > 0 && count($urlChunks) > 1) {
                echo "等待 {$this->delayBetweenBatches} 秒后处理下一批次..." . PHP_EOL;
                sleep($this->delayBetweenBatches);
            }
        }

        return [
            'ok' => $totalFailed === 0,
            'type' => 'deleted_articles',
            'summary' => [
                'total_urls' => count($deletedUrls),
                'total_success' => $totalSuccess,
                'total_failed' => $totalFailed,
                'total_batches' => count($urlChunks)
            ],
            'batch_details' => $batchResults
        ];
    }
}