...
class LibUpload
{
    ...
    public static function upload2Remote($id, $imgPath, $position, $remoteUrl = null, $_id = '')
    {
        ...
        $sign = (new LibCrypt())->make_sign($data, config('upload.img.key',  config('app.data_sync_key')));
        $data['cover'] = $cover;
        $data['sign'] = $sign;
        //print_r($data);
    ...
    {
        if ($remoteUrl === null) {
            $remoteUrl = config('upload.mp4_upload', 'http://video.ycomesc.live:2082/uploadMp4.php');
        }
        ...
        $data = [
            ...
            'video'    => $cover,
            'timestamp' => $timestamp,
            'sign' => md5($timestamp . config('upload.mp4_key', config('upload.mp4_key'))),
        ];
        return self::execCurl($remoteUrl, $data);
    }
